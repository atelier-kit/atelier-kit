...
All supported agents follow the same pattern:

1. read repository-local instructions generated by atelier-kit
2. translate chat planner commands into CLI commands
3. re-read `.atelier/context.md` after every state mutation
4. continue with the skill implied by the new planner state

The user gives planner commands in chat. The agent applies them through the CLI.

## The common command protocol

Inside an agent session, use these chat commands:
...
`/slice start` is an adapter shortcut, not a standalone CLI command.
Adapters translate it to `atelier-kit planner next` and continue in implementer
mode if a slice becomes active.
...
If the agent does not reload state, it may continue with the wrong task, slice,
or skill.
